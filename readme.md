...
## Install Duitku Nobu QRIS plugins through Composer
...
1. Install the plugins: `composer require duitku_payment_gateway/duitku_nobu_qris`
2. Enable the plugin:  `bin/magento module:enable Duitku_Nobuqris`
...
6. Check the module status:  `bin/magento module:status Duitku_Nobuqris`
...
## Install Duitku Nobu QRIS plugins from Download file

With these steps, you can custom/modify our Magento plugins to handle the business model that you want

1. Download and extract the plugin you have previously downloaded from GitHub and rename the folder as Nobuqris.
...
4. Move the Nobuqris folder into the Magento `root/app/code/Duitku` folder. </br>(You might need to create the folder `code/Duitku` if it is not exist).
5. Run this command on terminal

    `bin/magento module:enable Duitku_Nobuqris`
    
    `bin/magento setup:upgrade`
    
    `bin/magento cache:clean`
    
    `bin/magento module:status Duitku_Nobuqris`


# Plugin Usage Instruction
## Basic Plugins Configuration

Before you begin, make sure that you have successfully installed and enabled Duitku Nobu QRIS plugins.
Configure the Duitku Nobu QRIS plugin in your Magento admin panel: 
...
4. In the Other Payment Method section, click Duitku Nobu QRIS to extend Basic Settings and fill out the following fields:
